...
class File(db.Model):
    ...
    def remove_tag(self,tag_name):
        if db.files.find_one({'file_id':self.id,'tag':tag_name}).count()!=0:
    ...
    def tags(self):
        tagstrings = ''
        if db.files.find_one({'file_id':self.id,'tag':{'$exists':True}}).count()!=0:
            for file in db.files.find_one({'file_id':self.id}):
