...
#            exit()
    ...
    @property
    def if_need_help(self):
        for opt, val in self.opts:
            if opt == '-h' or opt == '--help':
                return 'Usage: calculator.py -C cityname -c configfile -d userdata -o resultdata'
...
class Config(object):
    ...
    def __init__(self):
        ...
        self.config.read(str(args.config_path))
        if args.cityname:
            self.config = self.config[args.cityname]
        else:
...
class IncomeTaxCalculator(object):
    ...
    def calc_for_all_userdata(self):
        ...
        for employee_id, income in self.userdata:
            ...
            data += [social_insurance_money, tax, remain, datetime.strftime(datetime.now(),'%Y-%m-%d %H:%M:%S')]
            result.append(data)
        # 返回所有用户的工资数据
        return result
